...
export class Consolidator {
   ...
   */
  dependsOnJobNameRegex(): { [key: string]: RegExp } {
   ...
   */
  async jobArtifacts(): Promise<{
    [k: string]: { [k: string]: ArtifactInfo | undefined };
  }> {
    ...
    return _.mapValues(dependsOnJobs, (jobs) => {
      return _.mapValues(jobs, (job: JobInfo) => {
        return this.artifacts.find((a) => a.name == job.id?.toString());
      });
    });
  }
  ...
  async filterForRelevantJobDetails(
    ...
    const endResults = _.reduce(
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (result: any, job) => {
        // Find if the job name matches any of the patterns.
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const jobMatches = _.toPairs(jobNameMatchers).find(([_name, matcher]) =>
          matcher.test(job.name)
        );
        if (jobMatches) {
          // eslint-disable-next-line @typescript-eslint/no-unused-vars
  ...
  async getJobOutputs(): Promise<{ [k: string]: any }> {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const jobResults: { [k: string]: { [k: string]: unknown } } = {};
    for (const jobKey of Object.keys(jobArtifacts)) {
      jobResults[jobKey] = {};
      const artifacts = jobArtifacts[jobKey];
